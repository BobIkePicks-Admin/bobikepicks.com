...
/** Download a PDF from storage (by its stored path) as a Buffer. */
export async function downloadPdfByPath(supa, path, name) {
  if (!path) return null;
  const { data, error } = await supa.storage.from(PICKS_BUCKET).download(path);
  if (error) throw error;
  const arrayBuf = await data.arrayBuffer();
  return { buffer: Buffer.from(arrayBuf), name: name || "BobIkePicks.pdf" };
}

/** Download the currently-published PDF from storage as a Buffer. */
export async function downloadCurrentPdf(supa, state) {
  return downloadPdfByPath(supa, state?.pdf_path, state?.pdf_name);
}
